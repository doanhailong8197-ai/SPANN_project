...
class SPANN_model:
    ...
    def __init__(self, x_dim, z_dim, enc, dec, class_num, device, cls_dropout=0.1):
        super().__init__()
            ...
            for minibatch_id, ((im_source, label_source, id_source), (im_target, label_target, id_target)) in enumerate(
                ...
                if iteration > miditer1:
                    ...
                    _, mc_uncertainty_train = mc_dropout_predict(
                        self.encoder, self.classifier, im_target, 0, y_target, n_samples=20
                    )
                    self.encoder.train()
                ...
                if iteration % test_freq == 0 or iteration == maxiter:
                    with torch.no_grad():
                        ...
                        for _, ((im_target, label_target, id_target)) in enumerate(test_target_cm_dl):
                            ...
                            _, mc_uncertainty_t = mc_dropout_predict(self.encoder, self.classifier, im_target, 0, y_target, n_samples=20)
                ...
                if iteration % test_freq == 0 or iteration == maxiter:
                    with torch.no_grad():
                        ...
                        for _, ((im_target, label_target, id_target)) in enumerate(test_target_cm_dl):
                            ...
                            # Gọi hàm dự đoán MC Dropout với n_samples=20
                            _, mc_uncertainty_t = mc_dropout_predict(self.encoder, self.classifier, im_target, 0, y_target, n_samples=20)
                            # Restore eval mode cho phần còn lại của test block
                            self.encoder.eval()
                            self.classifier.eval()
