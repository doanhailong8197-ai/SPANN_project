...
class CLS(nn.Module):
    def __init__(self, in_dim, out_dim, hidden_mlp=1024, feat_dim=16, temp=0.05, dropout=0.0):
        super(CLS, self).__init__()
        layers = [nn.Linear(in_dim, hidden_mlp), nn.ReLU(inplace=True)]
        if dropout > 0:
            layers.append(nn.Dropout(dropout))
        layers.append(nn.Linear(hidden_mlp, feat_dim))
...
def ensemble_score(logit_tensor, confidence_ot, mc_uncertainty=None):
    ...
    if mc_uncertainty is None:
        ensemble = (entropy_cls + softmax_cls + confidence_ot) / 3
    else:
        ensemble = (entropy_cls + softmax_cls + confidence_ot + (1 - mc_uncertainty)) / 4
    return ensemble, entropy_cls, softmax_cls, confidence_ot

def enable_mc_dropout(module):
    # Chỉ bật Dropout layer, không gọi module.eval() để tránh ảnh hưởng BatchNorm
    for m in module.modules():
        if isinstance(m, nn.Dropout):
            m.train()


def mc_dropout_predict(encoder, classifier, x, domain, y, n_samples=20):
    enable_mc_dropout(encoder)
    enable_mc_dropout(classifier)

    probs_samples = []
    with torch.no_grad():
        for _ in range(n_samples):
            z, mu, var = encoder(x, domain, y)
            _, logit = classifier(z)
            probs_samples.append(F.softmax(logit, dim=-1))
    probs_samples = torch.stack(probs_samples, dim=0)

    mean_probs = probs_samples.mean(dim=0)
    pred_var = probs_samples.var(dim=0).mean(dim=-1)
    mc_uncertainty = pred_var / (pred_var.max() + 1e-8)

    # Restore train mode sau khi MC Dropout
    encoder.train()
    classifier.train()

    return mean_probs, mc_uncertainty
...
class SPANN_model:
    ...
    def __init__(self, x_dim, z_dim, enc, dec, class_num, device, cls_dropout=0.1, n_samples=20):
        super().__init__()
        self.class_num = class_num
        self.device = device
        self.feat_dim = enc[-1][1]
        self.mc_n_samples = n_samples  # so lan forward MC Dropout, chinh duoc tu ngoai
        self.encoder = Encoder(x_dim, enc).to(device)
        self.decoder = Decoder(z_dim, dec).to(device)
        self.classifier = CLS(z_dim, class_num, dropout=cls_dropout).to(device)
        ...
        while iteration < maxiter:
            ...
            for minibatch_id, ((im_source, label_source, id_source), (im_target, label_target, id_target)) in enumerate(
                ...
                if iteration > miditer1:
                    ...
                    # Tính MC Dropout uncertainty để đưa vào E-score, nhất quán với eval
                    _, mc_uncertainty_train = mc_dropout_predict(
                        self.encoder, self.classifier, im_target, 0, y_target, n_samples=self.mc_n_samples
                    )
                    self.encoder.train()
                    self.classifier.train()
                    Escore_t, _, _, _ = ensemble_score(after_lincls_t, confidence_ot_all, mc_uncertainty=mc_uncertainty_train)
                    ###  calculate threshold
                ...
                if iteration % test_freq == 0 or iteration == maxiter:
                    with torch.no_grad():
                        ...
                        for _, ((im_target, label_target, id_target)) in enumerate(test_target_cm_dl):
                            ...
                            _, mc_uncertainty_t = mc_dropout_predict(self.encoder, self.classifier, im_target, 0, y_target, n_samples=self.mc_n_samples)
                            # Restore eval mode cho phần còn lại của test block
                            self.encoder.eval()
                            self.classifier.eval()

                            Escore_t, entropy_cls_t, softmax_cls_t, confidence_ot_t = ensemble_score(
                                after_lincls_t, confidence_ot_t, mc_uncertainty=mc_uncertainty_t
                            )
                ...
                if iteration % test_freq == 0 or iteration == maxiter:
                    with torch.no_grad():
                        ...
                        for _, ((im_target, label_target, id_target)) in enumerate(test_target_cm_dl):
                            ...
                            # Gọi hàm dự đoán MC Dropout với n_samples cấu hình được (self.mc_n_samples)
                            _, mc_uncertainty_t = mc_dropout_predict(self.encoder, self.classifier, im_target, 0, y_target, n_samples=self.mc_n_samples)
                            # Restore eval mode cho phần còn lại của test block
                            self.encoder.eval()
                            self.classifier.eval()
                            
                            # Cập nhật hàm tính điểm với tham số mới
                            Escore_t, entropy_cls_t, softmax_cls_t, confidence_ot_t = ensemble_score(
                                after_lincls_t, confidence_ot_t, mc_uncertainty=mc_uncertainty_t
                            )
